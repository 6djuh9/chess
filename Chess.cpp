...
void Board::printBoard() {
	...
	for (int i = 0; i < 8; i++)
	{
		...
		cout << " " << i + 1 << "   ";
		cout << endl << "     |____||____||____||____||____||____||____||____|" << endl;
	}
...
bool Board::doMove() {
	using namespace std;
	string move;
	int x1, x2, y1, y2, c1, c2, c3, c4;
	bool stop = false;
	while (!stop)
	{
		(turn == WHITE) ? cout << "White's turn" << endl : cout << "Black's turn" << endl;
		cout << "Type in your move as a single four character string. Use number-coordinates first in each pair." << endl;
		cout << "A-1, B-2, C-3,..., H-8." << endl;
		cin >> move;
		c1 = move[0] - 48;
		x1 = c1 - 1;
		c2 = move[1] - 48;
...
void Board::setBoard()
{
	...
	for (int j = 0; j < 8; j++)
	{
		square[1][j].setPieceAndColor(PAWN, WHITE);
		square[6][j].setPieceAndColor(PAWN, BLACK);
	...
	if (queenX != thatX || queenY != thatY)
	{
		...
		if (queenX == thatX)
		{
			...
			for (int i = queenY + yIncrement; i != thatY; i += yIncrement)
			{
				...
				if (square[i][thatX].getColor() != NONE)
					return false;
			...
			if (queenY == thatY)
			{
				...
				if (abs(queenX - thatX) == abs(queenY - thatY))
				{
					...
					for (int i = 1; i < abs(queenX - thatX); i++)
					{
						...
						if (square[queenY + yIncrement * i][queenX + xIncrement * i].getColor() != NONE)
							return false;
...
bool Board::moveBishop(Square* thisBishop, Square* thatSpace) {
	...
	if (abs(bishopX - thatX) == abs(bishopY - thatY))
	{
		...
		{
			std::cout << "It got here somehow";
			if (square[bishopY + yIncrement * i][bishopX + xIncrement * i].getColor() != NONE)
				return false;
